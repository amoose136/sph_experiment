...
%
%% Function Definition
function plot_trajectory( time, radius, theta, v_rad, v_theta, plist, varargin)
%% Input Arguments
...
%                                                               ( Default: true )
%   PlotRadius      plot_radius     Float        1          Add to plot the radius of the particle at the end position.
%   
%
%   v               v               Float        1          An array containing the all the particles that should have radii plotted
%   Cell            celli           Float        1          Specifies which cell to plot
%   GridData        grid_datai       Float        2          Contains all the grid data with all the radii in the second row and all the theta in the first row.
%   Sigma           tracer_sigma    Float        1          contains all sigma data corresponding to the tracer particles
%
%                                                               
%% Initialization
...
p.addOptional('v', [], ...
              @(x)validateattributes(x, {'numeric'}, ...
                                        {'vector', ...
                                         'real', ...
                                         'nonnegative'}));
p.addOptional('GridData_in', 1);
p.addOptional('Cell_in', 1);
p.addOptional('Sigma', 1);
...
plot_radius   = p.Results.PlotRadius;
v             = p.Results.v;
grid_datai    = p.Results.GridData_in;
celli         = p.Results.Cell_in;
tracer_sigma  = p.Results.Sigma;
...
if( ~isempty(cdata) )
    ...
    elseif( plot_fate )
        ...
        cdata_cmap(2,:)   = [255 0 255];
        cdata_cmap(3,:)   = [0 255 255];
        cdata_cmap(4,:)   = [0 255 0];
        cdata_cmap(5,:)   = [80 80 80];
        cdata_cmap(6,:)   = [0 0 255];
...
while k <= istop
    ...
    if( plot_velocity )
        quiver(axis_h(3), xplot(k,plist), yplot(k,plist), ...
                          ...
                          'MarkerSize', 3 );
    elseif( ~plot_cdata )
        scatter(axis_h(3), xplot(k,plist), yplot(k,plist), ...
                           16, ...
                           'filled', ...
                           'Marker', 'o', ...
                           'MarkerFaceColor', 'k');
    end
    % plot tracer radii if specified
    if ( plot_radius )
      for i= plist
        temprad=tracer_sigma(i)*3.29; %3.29 sigma should cover 99% of the area under the curve
        plot(rectangle('Position',[xplot(k,i)-temprad, yplot(k,i)-temprad, 2*temprad, 2*temprad],'Curvature',[1,1],'EdgeColor','b'))
        if (1)%sum(ismember(i,v))
          temprad=plot_radius(i);
          plot(rectangle('Position',[xplot(k,i)-temprad, yplot(k,i)-temprad, 2*temprad, 2*temprad],'Curvature',[1,1]))
        end
      end
    end
    % create plot of cell
    % if something
    for celli=0:131841
    cellj=mod(celli,257)+1;
    celli=mod(cellj,513)+1;
      coorda_x=grid_datai(2,celli)*cos(grid_datai(1,cellj));
      coorda_y=grid_datai(2,celli)*sin(grid_datai(1,cellj));
      coordb_x=grid_datai(2,celli)*cos(grid_datai(1,cellj+1));
      coordb_y=grid_datai(2,celli)*sin(grid_datai(1,cellj+1));
      coordc_x=grid_datai(2,celli+1)*cos(grid_datai(1,cellj+1));
      coordc_y=grid_datai(2,celli+1)*sin(grid_datai(1,cellj+1));
      coordd_x=grid_datai(2,celli+1)*cos(grid_datai(1,cellj));
      coordd_y=grid_datai(2,celli+1)*sin(grid_datai(1,cellj));
      plot([coorda_x,coordb_x,coordc_x,coordd_x,coorda_x],[coorda_y,coordb_y,coordc_y,coordd_y,coorda_y],'color','r','LineStyle','-','LineWidth',2);
    end
